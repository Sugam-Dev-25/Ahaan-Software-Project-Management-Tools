...
export const Sidebar = ({ collapsed, setCollapsed }: SidebarProps) => {
  ...
  return (
    <>
      ...
      >
        {/* LOGO */}
        <div className="h-18 flex items-center px-2 justify-left">
          <img
            src={collapsed ? "/Ahaan_PMS_Small.png" : "/Ahaan_PMS_Full.png"}
            alt="Ahaan Software"
            className={`transition-all duration-300 ${
              collapsed ? "h-12 w-12" : "h-24 w-auto"
            }`}
          />
        </div>
        <nav className="flex-1 px-2 py-3 space-y-1 overflow-y-auto">
          ...
          <NavLink
            to={`${dashboardBase}/tasks?scope=${role === "admin" || role === "super-admin" ? "all" : "mine"}`}
            className={({ isActive }) =>
              `${baseRow} ${isActive ? active : inactive} `
            }
          >
              ...
              {board.map((b) => {
                return (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block h-9 px-3 text-xs rounded-md capitalize flex items-center ${
                        isActive
